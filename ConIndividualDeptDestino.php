...
$queryq = "SELECT ot.staff_id ,concat_ws(' ',os.firstname,os.lastname)as nombre,count(ot.ticket_id ) as TotalCerrado,ots.state FROM ost_ticket ot
            ...
            left join ost_ticket_status ots on ots.id=ot.status_id
             left join ost_department od on os.dept_id=od.dept_id 
             where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and ot.dept_id='$VarIdDept' and ot.status_id='3' group by ot.staff_id order by nombre asc";   
...
$queryqq = "SELECT ot.staff_id ,concat_ws(' ',os.firstname,os.lastname)as nombress,count(ot.ticket_id ) as TotalAbierto,ots.state FROM ost_ticket ot
             ...
             left join ost_ticket_status ots on ots.id=ot.status_id
             left join ost_department od on os.dept_id=od.dept_id 
             where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and ot.dept_id='$VarIdDept' and ot.status_id='1' group by ot.staff_id order by nombress asc";   
...
$queryq = "SELECT ot.staff_id ,concat_ws(' ',os.firstname,os.lastname)as nombre,count(ot.ticket_id ) as TotalCerrado,ots.state FROM ost_ticket ot
            ...
            left join ost_ticket_status ots on ots.id=ot.status_id
             left join ost_department od on os.dept_id=od.dept_id 
             where ot.dept_id='$VarIdDept' and ot.status_id='3' group by ot.staff_id order by nombre asc";   
...
$queryqq = "SELECT ot.staff_id ,concat_ws(' ',os.firstname,os.lastname)as nombress,count(ot.ticket_id ) as TotalAbierto,ots.state FROM ost_ticket ot
             ...
             left join ost_ticket_status ots on ots.id=ot.status_id
             left join ost_department od on os.dept_id=od.dept_id 
             where ot.dept_id='$VarIdDept' and ot.status_id='1' group by ot.staff_id order by nombress asc";   
...
<table  border="1" cellspacing="0" cellpadding="2" align="center" >
    ...
    <tr>
     ...
     <center> <strong><p>Mostrar  los  <?php echo  $TotalFinalCerrado+$TotalFinalAbierto;?> Tickets <a href="http:DetallesDeptDestino.php">aquí</a></p></strong></center>
      </div>   
</td>
...
document.cookie ='VarCookieNombreAgente='+NombreAgente+';';
       ...
       //alert(NombreAgente);   
      window.open('DetallesAgentes.php','Detalles','width=1000, height=500')
...
document.cookie ='VarCookieNombreAgentess='+NombreAgentess+';';
      ...
      window.open('DetallesAgentesAbierto.php','Detalles','width=1000, height=500')
