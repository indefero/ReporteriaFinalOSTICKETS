           ...
           $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and  oli.extra='$VarExport' and ots.state='closed' and os.name='$VarExportAgente'and( od.dept_name like '%$Varnombre%' or (concat_ws(' ',osf.firstname,osf.lastname)like '%$Varnombre%') or ot.number like '%$Varnombre%' or oht.topic like '%$Varnombre%' )
                ...
                $resultado = mysqli_query( $conexion, $query );//variable resultada para almacenar el arreglo de la informacion tomando encuenta la conexion de la DB y el query requerido
                  ...
                  if ($resultado->num_rows>=1)// decicion que evalua el numero de registro afectados por el query en caso de ser verdadero imprime la tabla si no llamda a llamar un mensaje de error
                     {         
                      foreach ($resultado as $clave => $key) 
                           ...
                           echo ",<strong>\n\nRemitente:</strong>";echo $VarExportAgente;
                 ...
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and  oli.extra='$VarExport' and ots.state='closed' and os.name='$VarExportAgente'
                     order by ot.created desc ";
                $resultado = mysqli_query( $conexion, $query );//variable resultada para almacenar el arreglo de la informacion tomando encuenta la conexion de la DB y el query requerido
    ...
    // Query requerido para la bùsqueda por la caja de texto para los detalles
            $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where oli.extra='$VarExport' and ots.state='closed' and os.name='$VarExportAgente'and( od.dept_name like '%$Varnombre%' or (concat_ws(' ',osf.firstname,osf.lastname)like '%$Varnombre%') or ot.number like '%$Varnombre%' or oht.topic like '%$Varnombre%' )
                     order by ot.created desc ";
...
$query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where oli.extra='$VarExport' and ots.state='closed' and os.name='$VarExportAgente'
                     order by ot.created desc ";
