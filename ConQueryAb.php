<!--Este PHP  es encargado de controlar los Query que esten en estado abierto ya sea los primeros 50 ,100,200 o todos los tiket que esten abierto-->
...
// Desiciòn para cuando se presione el botòn de buscar
    ...
    {//Variable que guarda la informaciòn de la caja de texto a buscar
      ...
      // Query requerido para la bùsqueda por la caja de texto para los ticket abierto
            $query="SELECT ot.number,ot.created,os.name, concat_ws(' ',osf.firstname,osf.lastname)as nombre,od.dept_name,oli.value,oht.topic,ots.state,otp.priority_desc
                    FROM ost_ticket ot 
                        ...
                        where ots.state ='open' and (ot.number like '%$Varnombre%' or os.name like '%$Varnombre%' or concat_ws(' ',osf.firstname,osf.lastname)like '%$Varnombre%' or od.dept_name like '%$Varnombre%' or oli.value like '%$Varnombre%' or oht.topic like '%$Varnombre%' or otp.priority_desc like '%$Varnombre%')
  ...
  $queryT="SELECT ot.number  FROM ost_ticket ot ";
          ...
          require_once("LlenarTabla.php");//Llama al php de la tabla  para que se muestre en pantalla
            ...
            require_once("MsgError.php");//Llamada al php de error   
            }
    }
...
elseif(isset($_POST['btnAceptar']))
    ...
    if($VarConsulta == 1)
         {  //Query para el llenado de la tabla con los primeros 100 registros de ticket abierto
         $query="SELECT ot.number,ot.created,os.name, concat_ws(' ',osf.firstname,osf.lastname)as nombre,od.dept_name,oli.value,oht.topic,ots.state,otp.priority_desc
                    FROM ost_ticket ot 
                        left join ost_user os on ot.user_id=os.id 
                        left join ost_staff osf on ot.staff_id=osf.staff_id
        ...
        elseif($VarConsulta == 2)
             { //Query para el llenado de la tabla con los primeros 200 registros de ticket abierto
                 $query="SELECT ot.number,ot.created,os.name, concat_ws(' ',osf.firstname,osf.lastname)as nombre,od.dept_name,oli.value,oht.topic,ots.state,otp.priority_desc
                    FROM ost_ticket ot 
                        left join ost_user os on ot.user_id=os.id 
                        left join ost_staff osf on ot.staff_id=osf.staff_id
         ...
         elseif($VarConsulta == 0)
            {  //Query para el llenado de la tabla con todos los registros de ticket abierto
                  $query="SELECT ot.number,ot.created,os.name, concat_ws(' ',osf.firstname,osf.lastname)as nombre,od.dept_name,oli.value,oht.topic,ots.state,otp.priority_desc
                    FROM ost_ticket ot 
                        left join ost_user os on ot.user_id=os.id 
                        left join ost_staff osf on ot.staff_id=osf.staff_id
            ...
            {    //Query para el llenado de la tabla con los primeros 50 registros  abiertos  
                 $query="SELECT ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,od.dept_name,oli.value,oht.topic,ots.state,otp.priority_desc
                    FROM ost_ticket ot 
                        left join ost_user os on ot.user_id=os.id 
                        left join ost_staff osf on ot.staff_id=osf.staff_id
