<!--Este PHP  es encargado de controlar los Query para general las consultas de los ticket por temas de ayuda-->
...
if(isset($_POST['btnConsulta']))
      ...
      /*Query requerido para obtener el total de ticket por cada empleado con su nombre*/                    
             ...
             $strSql = "SELECT oht.topic as nombre,COUNT(ot.topic_id)as total from ost_ticket ot
                       inner join ost_help_topic oht on ot.topic_id=oht.topic_id
                        where (ot.created BETWEEN '$VarFechInicio' and '$VarFechFin') 
                        group by ot.topic_id
                        order by nombre asc";
