           ...
           $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq')and od.dept_name ='$VarExport' and ots.state='open' and (concat_ws(' ',osf.firstname,osf.lastname)='$VarExportAgente') and( oht.topic like '%$Varnombre%' or oli.extra like '%$Varnombre%' or os.name like '%$Varnombre%' or ot.number like '%$Varnombre%' )
                ...
                $resultado = mysqli_query( $conexion, $query );//variable resultada para almacenar el arreglo de la informacion tomando encuenta la conexion de la DB y el query requerido
                 ...
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and  od.dept_name ='$VarExport' and ots.state='open' and (concat_ws(' ',osf.firstname,osf.lastname)='$VarExportAgente')
                     order by ot.created desc ";
    ...
    // Query requerido para la bùsqueda por la caja de texto para los detalles
           $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where od.dept_name ='$VarExport' and ots.state='open' and (concat_ws(' ',osf.firstname,osf.lastname)='$VarExportAgente') and( oht.topic like '%$Varnombre%' or oli.extra like '%$Varnombre%' or os.name like '%$Varnombre%' or ot.number like '%$Varnombre%' )
                     order by ot.created desc ";
                ...
                $resultado = mysqli_query( $conexion, $query );//variable resultada para almacenar el arreglo de la informacion tomando encuenta la conexion de la DB y el query requerido
                 ...
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where od.dept_name ='$VarExport' and ots.state='open' and (concat_ws(' ',osf.firstname,osf.lastname)='$VarExportAgente')
                     order by ot.created desc ";
