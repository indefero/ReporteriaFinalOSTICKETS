...
$queryq = "SELECT oli.extra as nombre,count(ot.ticket_id ) as TotalCerrado,ots.state FROM ost_ticket ot
             ...
             where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ot.status_id='3' group by oli.extra order by nombre asc";   
  ...
  $queryqq = "SELECT oli.extra as nombress,count(ot.ticket_id ) as TotalAbierto,ots.state FROM ost_ticket ot
              left join ost_user os on ot.user_id=os.id 
                            left join ost_staff osf on ot.staff_id=osf.staff_id
                            left join ost_department od on ot.dept_id=od.dept_id 
                            left join ost_ticket__cdata otd on ot.ticket_id=otd.ticket_id
                            ...
                            left join ost_help_topic oht on ot.topic_id=oht.topic_id
                            left join ost_ticket_status ots on ot.status_id= ots.id
             where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ot.status_id='1' group by oli.extra order by nombress asc";   
...
$queryq = "SELECT oli.extra as nombre,count(ot.ticket_id ) as TotalCerrado,ots.state FROM ost_ticket ot
             left join ost_staff osf on ot.staff_id=osf.staff_id
                            ...
                            left join ost_help_topic oht on ot.topic_id=oht.topic_id
                            left join ost_ticket_status ots on ot.status_id= ots.id
             where concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ot.status_id='3' group by oli.extra order by nombre asc";   
...
$queryqq = "SELECT oli.extra as nombress,count(ot.ticket_id ) as TotalAbierto,ots.state FROM ost_ticket ot
              left join ost_user os on ot.user_id=os.id 
                            ...
                            left join ost_help_topic oht on ot.topic_id=oht.topic_id
                            left join ost_ticket_status ots on ot.status_id= ots.id
             where concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ot.status_id='1' group by oli.extra order by nombress asc";   
...
<table  border="1" cellspacing="0" cellpadding="2" align="center" >
    ...
    <tr>
     ...
     <center> <strong><p>Mostrar  los  <?php echo  $TotalFinalCerrado+$TotalFinalAbierto;?> Tickets <a href="http:DetallesEmpleado.php">aquí</a></p></strong></center>
      </div>   
</td>
...
echo "<strong><center>Tabla de Tickets Cerrados</center></strong>";
       echo"<table border=1 cellspacing=0 cellpadding=2 align='center' width=300>";// creacion de la tabla con los encabezados que se mostarar en el pantalla principal
   
            echo "<tr  align='center' bgcolor='bbbbbb' >
              
                 <td ALIGN=CENTER>Nº</td>
                 <td ALIGN=CENTER>Departamento de Origen</td>
...
document.cookie ='VarCookieNombreAgente='+NombreAgente+';';
       ...
       //alert(NombreAgente);   
      window.open('DetallesAgenteEmpleadoOrigen.php','Detalles','width=1000, height=500')
...
document.cookie ='VarCookieNombreAgentess='+NombreAgentess+';';
      ...
      window.open('DetalleIndividualEmpleadoAbOrigen.php','Detalles','width=1000, height=500')
