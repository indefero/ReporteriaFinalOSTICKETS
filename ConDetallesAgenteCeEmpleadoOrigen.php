...
if(isset($_POST['btnbuscar']))
 ...
 where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and  oli.extra='$VarExportAgente'and ots.state='closed'and concat_ws(' ',osf.firstname,osf.lastname)='$VarExport'and( od.dept_name like '%$Varnombre%' or os.name like '%$Varnombre%' or ot.number like '%$Varnombre%' or oht.topic like '%$Varnombre%' )
                ...
                $resultado = mysqli_query( $conexion, $query );//variable resultada para almacenar el arreglo de la informacion tomando encuenta la conexion de la DB y el query requerido
                     ...
                     {         
                      foreach ($resultado as $clave => $key) 
                           ...
                           echo "<strong>\n\nNº de Tickets:</strong>"; echo $VarFilas;  
                            echo ",<strong>\n\nAsignado a:</strong>"; echo $VarExport;
                           echo ",<strong>\n\nDepartamento de Origen:</strong>";echo $VarExportAgente;
                 ...
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
                     where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and  oli.extra='$VarExportAgente' and ots.state='closed' and  concat_ws(' ',osf.firstname,osf.lastname)='$VarExport'
                     order by ot.created desc ";
                $resultado = mysqli_query( $conexion, $query );//variable resultada para almacenar el arreglo de la informacion tomando encuenta la conexion de la DB y el query requerido
  ...
  $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
          where (ot.created BETWEEN '$VarExportInicio ' and ' $VarExportFinalq') and concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ots.state='closed' and oli.extra='$VarExportAgente'
                     order by ot.created desc ";
                  ...
                  if ($resultado->num_rows>=1)// decicion que evalua el numero de registro afectados por el query en caso de ser verdadero imprime la tabla si no llamda a llamar un mensaje de error
                     {         
                      ...
                      require_once("LlenarTablaDetalle.php");
                        ...
                        echo "<strong>El empleado no ingreso el departamento de origen</strong>";
                       }
    }
    ...
    // Query requerido para la bùsqueda por la caja de texto para los detalles
            $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
       where oli.extra='$VarExportAgente' and ots.state='closed' and concat_ws(' ',osf.firstname,osf.lastname)='$VarExportAgente'and( od.dept_name like '%$Varnombre%' or os.name like '%$Varnombre%' or ot.number like '%$Varnombre%' or oht.topic like '%$Varnombre%' )
                     order by ot.created desc ";
...
$query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
            where concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ots.state='closed' and oli.extra='$VarExportAgente'
                     order by ot.created desc ";
       ...
       $query = "SELECT ot.staff_id,ot.number,ot.created,os.name,concat_ws(' ',osf.firstname,osf.lastname)as nombre,oli.extra,od.dept_name,oht.topic,ots.state
                 FROM ost_ticket ot 
                    ...
                    left join ost_help_topic oht on ot.topic_id=oht.topic_id
                    left join ost_ticket_status ots on ot.status_id= ots.id
          where concat_ws(' ',osf.firstname,osf.lastname)='$VarExport' and ots.state='closed' and oli.extra='$VarExportAgente' order by ot.created desc ";
